...
**Требование к хосту: сетка МОЛЧА предполагает глобальный `*{box-sizing:border-box}`.** Колонки грида —
`width:100%` + боковые паддинги; собственного ресета стили пакета НЕ несут (единственный `box-sizing` в них —
`content-box` на yfm-типографике). Хост без глобального border-box = паддинги прибавляются к ширине →
горизонтальный оверфлоу на ВСЕХ ширинах + карточки сваливаются 2-в-ряд (verified двусторонне: инъекция
ресета → 0px, откат → 80px; `tsc`/build слепы, ловится только рендером). Наш scaffold закрывает это
layout-reset'ом (`scaffold-app-shell` «Глобальный reset») — строка здесь, чтобы факт был виден и с
PC-маршрута: чужой хост / голый embed без ресета сломается именно так.
