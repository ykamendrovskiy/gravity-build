...
Стили uikit **не** обнуляют отступ `body`, не растягивают корень на всю высоту и **не задают `box-sizing: border-box` глобально** (uikit ставит его только на `html`, дальше по дереву — браузерный `content-box`). Без reset: (1) `body { margin: 8px }` оставляет зазор, `AsideHeader` не прилегает к краям; (2) `content-box` ломает грид-раскладки, которые ждут border-box — напр. сетка `@gravity-ui/page-constructor` **молча предполагает глобальный border-box от хоста** (свои стили пакет ресетом не снабжает): паддинги колонок прибавляются к ширине → **горизонтальный оверфлоу на всех ширинах + карточки сваливаются 2-в-ряд вместо 3** (verified двусторонне: инъекция ресета → 0px, откат → 80px; см. `library-page-constructor` «Требование к хосту»); (3) `<ul>`/`<ol>` наследуют UA `padding-inline-start: 40px` — uikit `Stepper` это `<ol>`, без сброса **уезжает вправо на 40px** (verified). Добавь глобальный reset:
...
**Политика: панель — явный выбор пользователя** (сборка со сценарным манифестом → create-скилл
задаёт вопрос с выбором до скаффолда, см. его «Порядок работы»; рекомендация — «с панелью»):
в прод-билде она мертва вместе со сценариями, в дев/гейт-билде даёт просмотр состояний и тему —
цена нулевая, польза постоянная. Выбор уже прозвучал в запросе («с панелью» / «без панели») —
не переспрашивай; спросить некого (автономный / headless-прогон) — добавляй, это дефолт.
Отказ помечай в отчёте (как помечается эскейп «без проверки»); чисто статическая сборка без
манифеста панели не имеет по определению. Вид/состав ручек — профильная ось.
...
- **тема — тоже URL-параметром (`?theme=dark`)**: свап сценария перезагружает страницу — тема из одного
  лишь React-state его НЕ переживает. Читай `?theme=` при старте; переключатель пишет параметр через
  `history.replaceState` (без перезагрузки; дефолт-light не пиши — чистый URL); URL сценария строй от
  `location.href` — прочие параметры сохраняются сами (verified пробой: dark переживает свап сценария);
...
Text-подсказка «⌘+.»`; чип: `div[data-proto-panel] → Button view="raised" + Icon(Sliders)`. Маркер
вешай на реальный DOM-узел (надёжно — обёртка-`div`, как у чипа): не полагайся, что компонент
прокинет `data-*` на свой корень.
