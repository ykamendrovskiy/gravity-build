...
`ActionsPanel` + `ActionsPanelProps`. (verified uikit@7.49 source+tsc — `build/.../ActionsPanel/types.d.ts`)
...
Иконка действия — внутри `button.props.children` (отдельного `icon`-слота нет). (verified uikit@7.49 source)
