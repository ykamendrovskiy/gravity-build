...
`ActionsPanel` + `ActionsPanelProps`. (verified uikit@7.50 source+tsc — `build/.../ActionsPanel/types.d.ts`)
...
Иконка действия — внутри `button.props.children` (отдельного `icon`-слота нет). (verified uikit@7.50 source)
