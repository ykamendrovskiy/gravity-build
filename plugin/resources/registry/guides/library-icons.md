...
- **Размер: `Button` сайзит сам, остальные контролы — руками.** Кнопка оборачивает голую `<Icon>`/`<svg>` в
  `Button.Icon` и подставляет размер по своей шкале; свой `size` (или пара `width`+`height`) её переопределяет.
  Вне кнопки контрол иконку НЕ ресайзит (у `Icon` нет CSS-размера svg) → `Icon` без `size` падает на viewBox
  ≈16px, одинаковая иконка во всех размерах = баг. Ставь `<Icon size={N}/>` по шкале контрола.
...
`Label.iconSizeMap` (`Label/Label`) рулит только встроенными `copy`/`info`/`close`, не пользовательским `icon`;
у `Icon` нет CSS-размера svg (`Icon.css` = `line-height:0`, без `width`/`height`) → без `size` иконка падает на
viewBox ≈16px. Инпутообразные (`TextInput`/`NumberInput`/`PinInput`/date-*) слот иконку **не сайзят** —
контекста размера у них нет (про инсет старт-иконки — ниже).
