...
`library-uikit` (сам компонент) и `library-routing` («Иллюстрации статусов данных» — когда брать).
...
- `library-routing` — «Иллюстрации статусов данных» (когда брать пакет).
- `version-index` — версия `@gravity-ui/illustrations`.
