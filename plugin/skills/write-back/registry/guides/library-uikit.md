...
- **Шапка «крошки + правый контрол» — контракт приоритета: крошки уступают, контрол держится.** По
  ...
  (и левый бейдж/`Divider`) держатся — всем НЕ-крошкам `flex-shrink:0`. Тот же контракт flex-ряда, что выше
  (фикс-сосед `flex-shrink:0` / гибкий `min-width:0`); гибкий здесь — крошки, свёртка = их способ сжаться на
  реальном overflow. **Не усаживай `<ol>` ровно по контенту** (`Flex`-крошки без `flex:1`) — иначе ложная
