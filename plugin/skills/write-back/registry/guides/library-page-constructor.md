...
- **Выделение плана:** флага `popular`/`highlighted` у pricing-сабблоков НЕТ. Выделяй **парным** `theme`
  (`'default'|'dark'|'light'` — красит фон И текст вместе); **не** голым `backgroundColor` (меняет только фон →
  белый текст на светлом = невидим), и не миксуй `theme:'dark'` со светлым `backgroundColor`.
