...
| Обвязку приложения (Theme, Toaster, стили) | `recipes/app-shell.md` | — | обязательно |
...
Обвязка приложения (ThemeProvider, стили, опц. Toaster) — отдельный обязательный слой: `recipes/app-shell.md`.
...
Раз у таблицы есть selection-чекбоксы — при **непустом выборе** покажи `ActionsPanel` (`@gravity-ui/uikit`) с массовыми действиями: не оставляй выбор «мёртвым» (выбрал строки — должно быть видно, что с ними сделать). Без этого selection бесполезен.

Раскладка (критично — частые ошибки прогонов):
- **Закреплена к краю** (`sticky`/`fixed`), **не уезжает со скроллом** списка — действия над выбором видны всегда.
- **Компенсирующий отступ** списку (высота панели), иначе последние строки прячутся под ней.
- **Ширина синхронизирована с таблицей, к которой панель относится** — обычно **= ширине этой таблицы**, иногда меньше, **никогда больше**. (В `auto-width`/`sticky`-обёртке без заданной ширины действия наезжают друг на друга — дай контейнер с шириной таблицы.)

Детали — `recipes/library-uikit.md` → «Идиомы».
...
- `recipes/app-shell.md` — обязательная обвязка (ThemeProvider / стили / опц. Toaster).
- `r/library-routing.json` — выбор между тремя Gravity Table и остальными библиотеками.
