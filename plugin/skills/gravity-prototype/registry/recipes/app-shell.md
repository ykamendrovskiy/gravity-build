...
Прежде чем писать код — настрой конфиги проекта (`tsconfig.json`, `vite.config.ts`, build-скрипт). Бери их из официального шаблона **`gravity-ui-vite-example`** (`r/library-docs.json` → «Project scaffold»: github.com/gravity-ui/gravity-ui-vite-example) — у него сборка корректна. **Не сочиняй `tsconfig` руками.**
...
- Кастомная тема / бренд — отдельный слой поверх этого: см. `pattern-theming` (импорт `.theme.css` **после** `styles.css`; не переопределяй 2-4 токена руками).
...
- `library-routing` — когда какую библиотеку брать.
- `version-index` — версии пакетов.
- Recipes: `recipe-dashboard`, `recipe-landing`, `recipe-settings-form`.
